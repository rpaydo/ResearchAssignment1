...
The mean number of steps over all days with steps is `r meansteps`.  The median number of steps
over all days with steps is `r mediansteps`.  
...
The 5-minute interval with the highest average number of steps starts at `r max`.  
...
There are `r missing` rows in the data set that are missing values.  
...
With the imputed values, the mean number of steps over all days with steps is `r meanstepsimp`.  
The median number of steps over all days with steps is `r medianstepsimp`. This compares to the
previous mean of `r meansteps` and median of `r mediansteps`.
